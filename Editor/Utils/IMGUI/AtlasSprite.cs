...
namespace Smidgenomics.Unity.ProjectView.Editor
{
	...
	internal class _AtlasIcon : PropertyDrawer
	{
		...
		private static readonly (string, int, int)[]
		_OFFSET_BTNS = new (string, int, int)[]
		{
			("-x", -1, 0),
			("+x", 1, 0),
			("-y", 0, -1),
		...
		private static void PreviewIcon(in Rect rect, in DrawerContext ctx)
		{
			...
			var offset = ctx.offset.vector2Value;
			var size = ctx.size.vector2Value;
			IMGUI.Border(rect, Color.black);
			IMGUI.DrawRect(rect, Color.black * 0.2f);
			if (!tex) { return; }

			var ir = rect;
			ir.Resize(-2f);
			IMGUI.DrawSprite(ir, tex, offset, size);
		}
